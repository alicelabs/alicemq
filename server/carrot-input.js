...
function Carrot(config) {
  ...
  else this.uri = `http://${config.username}:${config.password}@${config.host}/api`;
}
  ...
  return new Promise((res, rej) => {
    fetch(this.uri + '/overview', this.options)
      ...
      .then((data) => {
        ...
        const result = { message_stats, cluster_name, queue_totals, object_totals };
        res(result);
      })
      .catch((err) => {
        console.error(err.stack);
        rej('Overview FAILED: ', err.stack);
      });
  });
};
    ...
    fetch(this.uri + '/exchanges', this.options)
      ...
      .then((data) => {
        const result = data.map((el) => {
          ...
          const result = { message_stats, name, type, durable };
      ...
      .catch((err) => {
        console.error(err.stack);
        rej('Exchanges FAILED: ', err.stack);
      });
  });
};
...
Carrot.prototype.consumers = function () {
  return new Promise((res, rej) => {
    fetch(this.uri + '/consumers', this.options)
      ...
      .catch((err) => {
        console.error(err.stack);
        rej('Consumers FAILED: ', err.stack);
      });
  });
};
