...
function carrot2D3(carrotData) {
  ...
  const calcWidth = window.innerWidth < 950 ? window.innerWidth : (window.innerWidth * 62) / 100;
  const calcHeight = window.innerWidth < 950 ? (parent.innerHeight * 90) / 100 : parent.innerHeight;
  ...
  function linkExchangeToQueues(b, _q) {
    b.forEach((binding) => {
      ...
      d3Data.nodes.forEach((node, i) => {
        if (node.name === queueName && node.group === 3) {
          const currentQueue = queues[queues.findIndex((el) => el.name === queueName)];
          const currentExchange = exchanges[exchanges.findIndex((el) => el.name === echangeName)];
          ...
          let message_rate = Math.min(
            currentQueue.message_stats.publish_details.rate,
            currentExchange.message_stats.publish_out_details.rate
          );
